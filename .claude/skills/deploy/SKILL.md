...
The resource key `agent_supervisor_chat` matches the app name in `databricks.yml` under `resources.apps`.
...
databricks bundle deployment bind agent_supervisor_chat <existing-app-name> --auto-approve
```
...
```bash
databricks bundle deployment unbind agent_supervisor_chat
```
...
| App running old code after deploy | Run `databricks bundle run agent_supervisor_chat` after deploy |
